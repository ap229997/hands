...
class Transformer(nn.Module):
    ...
    def forward(self, query, key, **kwargs):
        output = query
        for i, layer in enumerate(self.layers):
            output = layer(query=output, key=key, **kwargs)
...
class Block(nn.Module):
    ...
    def forward(self, query, key, query_embed=None, key_embed=None, **kwargs):
        b, c, h, w = query.shape
        ...
        if kwargs.get('kpe_emb', None) is not None:
            kpe_emb = kwargs['kpe_emb']
            kpe_emb = kpe_emb.permute(0,2,1).view(b, self.channels, h, w)
            q_embed = q_embed + kpe_emb
            k_embed = k_embed + kpe_emb
