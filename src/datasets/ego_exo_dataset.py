...
class EgoExoDataset(Dataset):
    def __init__(self, args, mode='test') -> None:
        ...
        filename = f"{os.environ['DATA_DIR']}/ego4d_hands/joint_annotations_egoexo_val.pkl"
        ...
        logger.info("# samples in EgoExo %s: %d" % (mode, len(self.subsampled_keys)))
    ...
    def __getitem__(self, index):
        ...
        joints3d_valid_r = np.array(joints3d_valid_r).copy()
        ...
        scale_original = max(data['image_size'][0], data['image_size'][1]) / 200.0
        center_original = [data['image_size'][0] / 2.0, data['image_size'][1] / 2.0]
        intrx = np.array(data['intrx'])
