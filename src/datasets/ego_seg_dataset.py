...
class Ego4DSegDataset(Dataset):
    def __init__(self, args, mode='train') -> None:
        ...
        self.args = args
        self.aug_data = 'train' in mode # remove False later, just for debugging
        self.normalize_img = Normalize(mean=args.img_norm_mean, std=args.img_norm_std)
        self.egocam_k = None

        bbox_file = f"{os.environ['DATA_DIR']}/ego4d_hands/grasp_ego.pkl"
        with open(bbox_file, 'rb') as f:
            self.bbox_data = pickle.load(f)

        if args.get('use_gt_hand_mask', False):
            raise NotImplementedError("GT hand mask not available for Ego4D")
        else:
            masks_npz_file = f"{os.environ['DATA_DIR']}/ego4d_hands/ego_blur_pred_masks.npz"
        self.masks_npz = np.load(masks_npz_file, allow_pickle=True)

        self.imgnames = list(set(self.bbox_data.keys()) & set(self.masks_npz.files))

        # subsample indices
        all_keys = list(range(len(self.imgnames)))
        self.subsampled_keys = sorted(downsample(all_keys, mode))

        logger.info("# samples in Ego4D Seg %s: %d" % (mode, len(self.subsampled_keys)))
    ...
    def __getitem__(self, index):
        ...
        if args.get('use_render_seg_loss', False): # check if mask is corrupt
        ...
        modal_labels = {'left': 1, 'right': 1} # assume everything is unoccluded for now
