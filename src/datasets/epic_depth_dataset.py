...
class EPICDepthDataset(Dataset):
    def __init__(self, args, mode='train') -> None:
        ...
        depth_dir = f"{os.environ['DATA_DIR']}/visor_depth" # store ZoeDepth predictions on VISOR images here
        self.depth_data = {}
        for imgname in self.bbox_data.keys():
            fileid = imgname.split('/')[-1].replace('jpg', 'png')
            depth_path = op.join(depth_dir, fileid)
            if op.exists(depth_path):
                self.depth_data[imgname] = depth_path

        self.imgnames = list(set(self.bbox_data.keys()) & set(self.depth_data.keys()))

        # subsample indices
        all_keys = list(range(len(self.imgnames)))
        self.subsampled_keys = sorted(downsample(all_keys, mode))

        logger.info("# samples in EPIC Depth %s: %d" % (mode, len(self.subsampled_keys)))
    ...
    def __getitem__(self, index):
        ...
        if args.get('use_depth_loss', False):
            # process depth
            depth = np.array(Image.open(self.depth_data[imgname])) / 1000.0 # mm to m
            augm_dict_depth = augm_dict.copy()
            augm_dict_depth["pn"] = np.ones_like(augm_dict_depth["pn"]) # depth is already noisy
            depth_img = data_utils.depth_processing(
                    self.aug_data,
                    depth,
                    center,
                    scale,
            ...
            mask_bbox_r = np.zeros_like(depth_img)
            if right_bbox is not None:
                mask_bbox_r[r_bbox[1]:r_bbox[3], r_bbox[0]:r_bbox[2]] = 1
            mask_bbox_l = np.zeros_like(depth_img)
            if left_bbox is not None:
                mask_bbox_l[l_bbox[1]:l_bbox[3], l_bbox[0]:l_bbox[2]] = 1
            depth_r = depth_img * mask_bbox_r
            depth_l = depth_img * mask_bbox_l
            targets['depth.r'] = torch.from_numpy(depth_r).float()
            targets['depth.l'] = torch.from_numpy(depth_l).float()
            meta_info['is_depth_loss'] = 1
