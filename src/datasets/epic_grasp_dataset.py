...
class EPICGraspDataset(Dataset):
    def __init__(self, args, mode='test') -> None:
        ...
        filename = f"{os.environ['DATA_DIR']}/epic_hands/grasp_visor_train.pkl"
        ...
        logger.info("# samples in EPIC Grasp %s: %d" % (mode, len(self.subsampled_keys)))
    ...
    def __getitem__(self, index):
        ...
        meta_info['is_grasp_loss'] = 1

        if data['left_grasp'] is None:
            targets["grasp.l"] = 8 # no grasp
        else:
            targets["grasp.l"] = self.grasp_label[data['left_grasp']]
        if data['right_grasp'] is None:
            targets["grasp.r"] = 8 # no grasp
        else:
            targets["grasp.r"] = self.grasp_label[data['right_grasp']]
