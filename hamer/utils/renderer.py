...
class Renderer:
    ...
    def __init__(self, cfg: CfgNode, faces: np.array):
        ...
        """
        self.cfg = cfg
        self.focal_length = cfg.get('focal_length') if cfg.get('focal_length') is not None else cfg.EXTRA.FOCAL_LENGTH
        self.img_res = cfg.get('img_res') if cfg.get('img_res') is not None else cfg.MODEL.IMAGE_SIZE
