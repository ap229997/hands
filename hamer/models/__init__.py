...
def load_hamer(checkpoint_path=DEFAULT_CHECKPOINT):
    ...
    # set paths here
    model_cfg.defrost()
    model_cfg.MANO.DATA_DIR = os.environ['HAMER_MANO_DIR']
    model_cfg.MANO.MODEL_PATH = os.path.join(model_cfg.MANO.DATA_DIR, 'mano')
    model_cfg.MANO.MEAN_PARAMS = os.path.join(model_cfg.MANO.DATA_DIR, 'mano_mean_params.npz')
    model_cfg.freeze()
